import type { Entry$input, Entry$result, QueryStore, QueryStoreFetchParams} from '$houdini'

export declare class EntryStore extends QueryStore<Entry$result, Entry$input> {
	/**
	 * ### Route Loads
	 ...
	 * 	const store = new EntryStoreStore()
 ...
 *
 * ```js
 * import { load_Entry } from '$houdini';
 ...
 *   };
 *
 *   return await load_Entry({ event, variables });
 * };
 * ```
 *
 ...
 *
 * ```js
 * import { loadAll, load_Entry } from '$houdini';
 * import type { PageLoad } from './$types';
 *
 * export const load: PageLoad = async (event) => {
 *   const variables = {
 ...
 *     load_Entry({ event, variables }),
 *     // load_ANOTHER_STORE
 *   );
 * };
 * ```
 */
export declare const load_Entry: (params: QueryStoreFetchParams<Entry$result, Entry$input>) => Promise<{Entry: EntryStore}>
