import type { Ask$input, Ask$result, QueryStore, QueryStoreFetchParams} from '$houdini'

export declare class AskStore extends QueryStore<Ask$result, Ask$input> {
	/**
	 * ### Route Loads
 ...
 *   };
 *
 *   return await load_Ask({ event, variables });
 * };
 * ```
 *
 ...
 *
 * ```js
 * import { loadAll, load_Ask } from '$houdini';
 * import type { PageLoad } from './$types';
 *
 * export const load: PageLoad = async (event) => {
 *   const variables = {
 ...
 *     load_Ask({ event, variables }),
 *     // load_ANOTHER_STORE
 *   );
 * };
 * ```
 */
export declare const load_Ask: (params: QueryStoreFetchParams<Ask$result, Ask$input>) => Promise<{Ask: AskStore}>
